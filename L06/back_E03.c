...
Tratta **copiaTrattaInOrdinamentiTratta( Tratta *t, int n);
Tratta **mallocOrdinamentiTratta( Tratta *t, int n);
bool leggiComando( comando_e *cmd, tipo_output *output_t);
Tratta *openNewFile( int *n, char *input_file_name);
...
int main(void)
{
	...
	tratta = openNewFile( &n_tratte, input_file_name);
	for( int i = 0; i < n_ordinamenti; i++)
		ordinamenti_tratta[i] = copiaTrattaInOrdinamentiTratta( tratta, n_tratte);
...
{
	Tratta **ord_t;
	ord_t = mallocOrdinamentiTratta( t, n);
