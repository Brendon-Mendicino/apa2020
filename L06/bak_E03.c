...
Tratta *salva_log( FILE *input, int *n_tratte);
void copiaTrattaInOrdinamentiTratta( Tratta *t, Tratta **(*ord_t)[n_ordinamenti], int n);
Tratta **mallocOrdinamentiTratta( Tratta *t, int n);
bool leggiComando( comando_e *cmd, tipo_output *output_t);
void openNewFile( Tratta **t, int *n, char *input_file_name);
...
int main(void)
{
	...
	openNewFile( &tratta, &n_tratte, input_file_name);
	copiaTrattaInOrdinamentiTratta( tratta, &ordinamenti_tratta, n_tratte);
...
Tratta *salva_log( FILE *input, int *n_tratte)
{
	/* Alloco dinamicamente '*tratta' */
	int i = 0;
	char data_buff[MAX_STR] = {0};
	Tratta *tratta;

	fscanf( input, "%d", n_tratte);
	if( (tratta = (Tratta *)malloc(*n_tratte * sizeof(Tratta))) == NULL)
		ERR_out_of_mem();
...
void copiaTrattaInOrdinamentiTratta( Tratta *t, Tratta **(*ord_t)[n_ordinamenti], int n)
{
	/* Uso un puntatore al vettore e alloco la memoria */
	for( int i = 0; i < n_ordinamenti; i++)
		(*ord_t)[i] = mallocOrdinamentiTratta( t, n);
}

Tratta **mallocOrdinamentiTratta( Tratta *t, int n)
{
	Tratta **ord_t;
	if( (ord_t = (Tratta **)malloc(n * sizeof(Tratta *)) ) == NULL)
		ERR_out_of_mem();
...
void openNewFile( Tratta **t, int *n, char *input_file_name)
{
	...
	apri_file( &input, input_file_name);
	*t = salva_log( input, n);
